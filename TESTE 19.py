...
def initialize_rpi_connection():
    rpi_ip = '192.168.100.149'  # Substitua pelo IP da sua Raspberry Pi 4
...
def detect_head_movements():
    global initial_nose_y, COUNTER, TOTAL_BLINKS, is_locked, last_blink_time, last_movement_time
    global rpi_sock  # Adicionado para acessar a variável global
            ...
            for face in faces:
                ...
                if not is_locked and last_movement_time is not None:
                    if (time.time() - last_movement_time) > LOCK_TIMEOUT:
                        ...
                        send_command_to_both(0, 0)  # Enviar comando de parada
                ...
                if not is_locked:
                    if movement_detected:
                        ...
                        # Envia os comandos proporcionais para ambos os sistemas
                        send_command_to_both(left_wheel_speed, right_wheel_speed)
                        ...
                        send_command_to_both(0, 0)  # Envia comando de parada
...
# Fechar a conexão com o CoppeliaSim e a Raspberry Pi 4
