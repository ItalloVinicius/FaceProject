...
import socket  # Importar a biblioteca socket para comunicação com o ESP8266
import atexit  # Para garantir que o socket seja fechado ao sair
...
ESP8266_IP = '192.168.0.123'  # Substitua pelo endereço IP do seu ESP8266
ESP8266_PORT = 8080

# Criar uma conexão socket com o ESP8266
...
try:
    ...
    print(f"Falha ao conectar ao ESP8266: {e}")
    sys.exit(1)

# Função para enviar comando ao CoppeliaSim e ao ESP8266
def send_coppelia_command(left_wheel_speed, right_wheel_speed):
    command_str = f"{left_wheel_speed},{right_wheel_speed}"
    # Enviar para o CoppeliaSim
    try:
        sim.simxSetStringSignal(clientID, 'wheelchair_command', command_str, sim.simx_opmode_oneshot)
        print(f"Comando enviado para o CoppeliaSim: {command_str}")
    except Exception as e:
        print(f"Erro ao enviar comando para o CoppeliaSim: {e}")

    # Enviar para o ESP8266
    try:
        esp_socket.sendall((command_str + '\n').encode('utf-8'))
        print(f"Comando enviado para o ESP8266: {command_str}")
    except (BrokenPipeError, ConnectionResetError):
        print("Conexão com o ESP8266 perdida. Tentando reconectar...")
        reconnect_esp8266()
    except Exception as e:
        print(f"Erro ao enviar comando para o ESP8266: {e}")

def reconnect_esp8266():
    global esp_socket
    esp_socket.close()
    esp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    while True:
        try:
            esp_socket.connect((ESP8266_IP, ESP8266_PORT))
            print("Reconectado ao ESP8266")
            break
        except Exception as e:
            print(f"Falha na reconexão: {e}. Tentando novamente em 5 segundos...")
            time.sleep(5)
