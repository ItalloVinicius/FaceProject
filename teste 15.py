...
def get_direction_names(yaw_control, pitch_control):
    ...
    elif 0.1 <= pitch_control < 0.5:
        pitch_name = "Avançando"
    elif -0.1 < pitch_control < 0.1:
        pitch_name = "Parado"
    elif -0.5 <= pitch_control <= -0.1:
        pitch_name = "Recuando"
    elif pitch_control <= -0.5:
...
def detect_head_movements():
    ...
    dead_zone_threshold = 0.2  # Limite da zona morta (ajuste conforme necessário)
    ...
    while True:
        ...
        if len(faces) > 0:
            for face in faces:
                ...
                if abs(yaw_control) < dead_zone_threshold:
                    yaw_control = 0.0
                if abs(pitch_control) < dead_zone_threshold:
                    pitch_control = 0.0

                # Calcula as velocidades das rodas esquerda e direita para controle diferencial
                max_speed = 5.0  # Velocidade máxima das rodas
